    ...
    for(int i=(int)std::max(0.0f,boundingBox.l);i<(int)std::min((float)width,boundingBox.r+1.0f);i++){
        for(int j=(int)std::max(0.0f,boundingBox.b);j<(int)std::min((float)height,boundingBox.t+1.0f);j++){
            if(insideTriangle(i,j,v)){
                ...
                int hit_place = get_index(i,j);
                if(z_interpolated<depth_buf[hit_place]){
                    Eigen::Vector3f c_interpolated = alpha * t.color[0] / v[0].w() + beta * t.color[1] / v[1].w() + gamma * t.color[2]/ v[2].w();
                    c_interpolated*=w_reciprocal;
                    // c_interpolated*=255.0f;

                    Eigen::Vector3f n_interpolated = alpha * t.normal[0]/ v[0].w() + beta * t.normal[1]/ v[1].w() + gamma * t.normal[2]/ v[2].w();
                    n_interpolated*=w_reciprocal;

                    Eigen::Vector2f t_interpolated = alpha * t.tex_coords[0]/ v[0].w() + beta * t.tex_coords[1]/ v[1].w() + gamma * t.tex_coords[2]/ v[2].w();
                    t_interpolated*=w_reciprocal;

                    Eigen::Vector3f p_interpolated = alpha * view_pos[0]/ v[0].w() + beta * view_pos[1]/ v[1].w() + gamma * view_pos[2]/ v[2].w();
                    p_interpolated*=w_reciprocal;

                    fragment_shader_payload payload( c_interpolated, n_interpolated.normalized(), t_interpolated, texture ? &*texture : nullptr);
                    payload.view_pos = p_interpolated;
